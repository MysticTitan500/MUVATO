...
string addItem(libSet setInQuestion, string itemName){
    string titStr;
    cout << "Please enter the name of the category you are adding " +itemName+ " to: " << endl;
    cin >> titStr;

    string newPar = setInQuestion.searchTitl(titStr).getName();

    if(setInQuestion.searchTitl(titStr).getKid()< 1){
        setInQuestion.createChild(itemName, titStr, setInQuestion.searchTitl(titStr));
        setInQuestion.searchTitl(titStr).addKid();

    }
    else{
        setInQuestion.createSibling(itemName, titStr, setInQuestion.searchTitl(titStr));
        setInQuestion.searchTitl(titStr).addKid();
    }

    string itemAdded = "Congratulations! "+itemName + " has been added!";
    return itemAdded;
}
...
int main() {
    ...
    for(int cntr=1; cntr <= setNum; cntr++){
        ...
        if(cntr <= 1){
            ...
            mainSet.createChild(catName, setName, mainSet.getRoot());
        }
        else{
            cout<< "Please enter the name of the next catagory: "<< endl;
            cin >> catName;

            mainSet.createSibling(catName, setName, mainSet.getRoot());
        }
    ...
    cin >> option;
        ...
        if(option == 1){
            ...
            cout << "Parent: " + holderMemb.getParent() << endl;
